...
- Keep both supplied files together: `gradpack-0.1.0-alpha.1.zip` and its
  `.sha256` sidecar.
...
```bash
shasum -a 256 -c gradpack-0.1.0-alpha.1.zip.sha256
```

### Linux

```bash
sha256sum -c gradpack-0.1.0-alpha.1.zip.sha256
```

### Windows PowerShell

```powershell
Get-FileHash .\gradpack-0.1.0-alpha.1.zip -Algorithm SHA256
Get-Content .\gradpack-0.1.0-alpha.1.zip.sha256
```
